...
def list_training(count, id, name, started_at, data_set, memo, status, entry_point, tag):
    ...
    pprint.pp_table(['id', 'name', 'started_at', 'dataset', 'memo', 'tags', 'status'],
                    [[x.id, x.name, x.created_at, x.data_set.name, x.memo, x.tags, x.status] for x in result[:count]])
...
def create(name, registry_image, registry_tag, data_set_id, entry_point,
           ...
           tags, zip, parent_id, options, registry_id, git_id):
    """Submit new training"""
    api = rest.TrainingApi(configuration.get_api_client())
    ...
    model = rest.TrainingApiModelsCreateInputModel(
        ...
        gpu=gpu, memo=memo, memory=memory, name=name, options=option_dict, parent_id=parent_id, partition=partition, tags=list(tags), zip=zip)
