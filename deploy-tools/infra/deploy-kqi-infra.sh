...
get_current_time(){
  CURRENT_TIME=`data +"%Y%m%d%H%M"`
  LOCAL_CURRENT_TIME="local_current_time=$CURRENT_TIME"
}
...
check(){
  get_current_time
  ansible-playbook -i conf/inventory -e "$EXTRA_ARG" -e "$LOCAL_CURRENT_TIME" check-cluster.yml -b
}
