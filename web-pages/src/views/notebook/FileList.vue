<template>
  <div>
    ...
    >
      ...
      <el-row>
        <el-col :span="6">
          <pl-display-text-form label="学習名" :value="job.name" />
        </el-col>
        <el-col :span="6">
            ...
            >
              ...
              <span v-if="d !== ``">
                <el-button
                  ...
                  style="font-size: 1.8rem;"
                  @click="
                    handleNav(navigation.slice(0, idx + 1).join('/') + '/')
                  "
                  >{{ d }}</el-button
                >
              </span>
              ...
              <el-table-column prop="name" label="ファイル名" width="auto">
                <template slot-scope="scope">
                  <span v-if="scope.row.isDirectory">
                    <el-button
                      type="text"
                      @click="handleNavAdd(scope.row.name)"
...
export default {
  ...
  methods: {
    async getDetail() {
      try {
        this.job = (await api.notebook.getById({ id: this.id })).data
        this.error = null
      } catch (e) {
        this.error = e
      }
    ...
    async getFileList() {
      try {
        ...
        let data = (await api.notebook.getContainerFilesById(params)).data
        let newList = []
        data.dirs.forEach(d =>
          newList.push({
            isDirectory: true,
