<template>
  <div>
    <el-form-item label="レジストリ" prop="registryIds">
      <el-select class="selectRegistry" :value="value" @change="handleChange" multiple placeholder="Select"
                 :clearable="true">
        <el-option
          v-for="item in list"
          :key="item.id"
    ...
    <el-form-item label="デフォルト" prop="defaultRegistryId">
      <el-select class="selectRegistry" :value="selectedDefaultId" @change="handleChangeDefaultId" placeholder="Select"
                 :clearable="true">
        <el-option
