  ...
  >
    <el-form ref="form">
      ...
      <el-card>
        ...
        <el-form-item v-if="this.form.node" label="ログ">
          <br clear="all" />
          <span v-if="!filename">
  ...
  methods: {
    async changeId() {
      try {
        ...
        if (
          ...
          this.events = (
            await api.resource.admin.getContainerEventsByName(params)
          ).data
        }
        this.error = null
      } catch (e) {
    ...
    async handleDonwloadLog() {
      try {
        ...
        let content = (await api.resource.admin.getContainerLogByName(params))
          .data
