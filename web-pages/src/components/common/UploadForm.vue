...
<template>
  <el-progress v-if="showProgress" type="circle" :percentage="progress" class="progress"/>

  <div v-else>
    <input type="file" @change="selectFiles" name="files" ref="uploadInputTag" multiple/>
  </div>
    ...
    methods: {
      ...
      getTotalPart (selectedFiles) {
        ...
        for (let i = 0; i < selectedFiles.length; i++) {
          this.totalPart += Math.ceil(selectedFiles[i].size / MultiPartUpload.chunkSize)
        }
      }
    }
  }
