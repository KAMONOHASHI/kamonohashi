...
<template>
  <div class="el-input">
    ...
    <el-row>
      ...
      <el-col :span="12" v-if="enableCommitIdSelecter">
        <el-popover
          ...
          placement="right">
        <span>
          <pl-display-text-form label="コミットID" :value="selectedCommit.commitId"></pl-display-text-form>
          <pl-display-text-form label="コミッター" :value="selectedCommit.committerName"></pl-display-text-form>
          <pl-display-text-form label="コミット日時" :value="selectedCommit.commitAt"></pl-display-text-form>
          <pl-display-text-form label="コメント" :value="selectedCommit.comment"></pl-display-text-form>
        </span>
        </el-popover>
        <el-select
          size="small"
  ...
  export default {
    ...
    methods: {
      setSelectedParameter () {
        ...
        // repository
        if (this.value.owner !== undefined && this.value.repository !== undefined) {
      ...
      async getData () {
        ...
        if (this.selectedRepository.owner !== null && this.selectedRepository.name !== null) {
      ...
      async getRepositories () {
        ...
        try {
          ...
          if (this.repositories.length === 0) {
            ...
            if (this.selectedRepository.fullName && this.repositories.some(
      ...
      changeGitId (gitId) {
        ...
        if (gitId) { // バツボタンを押してリポジトリを削除した場合はFalseになる
