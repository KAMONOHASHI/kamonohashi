      ...
      <el-form ref="runForm" :rules="rules" :model="this">
        ...
        <div v-if="originId !== undefined">
          ...
          <el-row class="right-button-group footer">
            ...
            <el-button type="primary" @click="runInference">実行 </el-button>
          </el-row>
        </div>
        <div v-else>
          <el-row :gutter="20">
            <el-steps :active="active" align-center>
              <el-step
                title="Step 1"
                description="Inference name & dataset"
              ></el-step>
              <el-step
                title="Step 2"
            ...
            <div class="element">
              <el-form v-if="active === 0">
                <el-col :span="12">
                  <el-form-item label="推論名" prop="name" class="is-required">
                    <el-input v-model="name" />
                  </el-form-item>
                  <el-form-item label="マウントする学習" prop="parent">
                    <pl-training-history-to-mount-selector v-model="parent" />
...
export default {
  ...
  methods: {
      ...
      await form.validate(async valid => {
        if (valid) {
          try {
            ...
            await api.inference.post({ model: param })
    ...
    async copyFromOrigin() {
      let origin = (await api.inference.getById({ id: this.originId })).data
