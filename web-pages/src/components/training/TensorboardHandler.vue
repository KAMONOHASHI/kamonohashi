  ...
  <div class="el-input">
    ...
    <span v-else-if="statusName === 'Running'">
      <div v-if="current.url" class="tensorBoardlink">
        <el-button type="primary" @click="openTensorBoard" size="small" plain>開く</el-button>
        <el-button type="danger" @click="deleteTensorBoard" size="small" plain>停止</el-button>
      </div>
      <div v-else>
        利用可能リソース待機中...
      </div>
  ...
  export default {
    ...
    methods: {
      ...
      async checkTensorBoardStatus () {
        if (this.polling) { // 既にポーリング中だったら無視
          return
        }
        ...
        let next = (await api.training.getTensorboardById({
          id: this.trainingHistoryId,
          $config: {apiDisabledLoading: true}
        })).data
        if (this.current === null || this.current.statusType !== next.statusType) {
