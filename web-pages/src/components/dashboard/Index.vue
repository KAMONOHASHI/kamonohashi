<template>
  <div>
    <h2> ダッシュボード </h2>
    <div class="dashboard">
      <div class="dashboard-menu-list card-container" v-for="(menu, index) in menuList" :key="index">
        <router-link :to=menu.url>
          <el-card class=menu
                   style="border:solid 1px #ebeef5; width:360px;height: 200px; border-left: 5px solid rgba(26, 191, 213, 0.7);">
            <div class=menu-name>
              <icon v-if="menu.category" :name="menu.category" scale="1.5" class="menu-icon"></icon>
              {{menu.name}}
            </div>
            <div class=menu-description style="padding:10px;font-size:14px;">{{menu.description }}</div>
          </el-card>
        </router-link>
      </div>
    </div>
